...
void afficherMaze(ALLEGRO_BITMAP* laby_texture){
    ...
    for(int y = 0; y < ROWS; y++){
        for(int x = 0; x < COLS; x++){
            ...
            if(x!=0 && (grid[y][x] == 4 || grid[y][x] == 8)){ // A GAUCHE
                al_draw_tinted_scaled_rotated_bitmap_region(laby_texture, 72, 0, 24, 72, al_map_rgb(255,255,255), 0, 0, current_blue.x-LABY_WALL_SIZE, current_blue.y, 1, 1, 0, 0);
                ...
                al_draw_tinted_scaled_rotated_bitmap_region(laby_texture, 72, 0, 24, 72, al_map_rgb(255,255,255), 0, 0, current_blue.x+LABY_CASE_SIZE-LABY_WALL_SIZE, current_blue.y, 1, 1, 0, 0);
    ...
    for(int y = 0; y < ROWS; y++){ // les angles
        for(int x = 0; x < COLS; x++){
            current_blue.x = x*LABY_CASE_SIZE + X_PLATEAU+LABY_WALL_SIZE;
            current_blue.y = y*LABY_CASE_SIZE + Y_PLATEAU+LABY_WALL_SIZE;
            current_red.x = X_PLATEAU+LABY_CASE_SIZE*ROWS*2-LABY_WALL_SIZE - y*LABY_CASE_SIZE;
            ...
            if(((grid[y][x] == 8) && (y!=0 && grid[y-1][x] == 7)) || ((x!=0 && grid[y][x-1] == 6) && (grid[y][x] == 5)) || ((x!=0 && grid[y][x-1] == 6) && (y!=0 && grid[y-1][x] == 7))){ // A GAUCHE ET EN HAUT
                al_draw_tinted_scaled_rotated_bitmap_region(laby_texture, 84, 72, 12, 36, al_map_rgb(255,255,255), 0, 0, current_blue.x, current_blue.y-LABY_WALL_SIZE, 1, 1, 0, 0);
            }
            if(((grid[y][x] == 6) && (y!=0 && grid[y-1][x] == 7)) || ((grid[y][x] == 5) && (x != COLS-1 && grid[y][x+1] == 8)) || ((y!=0 && grid[y-1][x] == 7) && (x != COLS-1 && grid[y][x+1] == 8))){ // A DROITE ET EN HAUT
                al_draw_tinted_scaled_rotated_bitmap_region(laby_texture, 96, 72, 12, 36, al_map_rgb(255,255,255), 0, 0, current_blue.x+LABY_CASE_SIZE-LABY_WALL_SIZE, current_blue.y-LABY_WALL_SIZE, 1, 1, 0, 0);
            }
            if(((grid[y][x] == 8) && (y!=ROWS-1 && grid[y+1][x] == 5)) || ((x!=0 && grid[y][x-1] == 6) && (grid[y][x] == 7)) || ((y!=ROWS-1 && grid[y+1][x] == 5) && (x!=0 && grid[y][x-1] == 6))){ // A GAUCHE ET EN BAS
                al_draw_tinted_scaled_rotated_bitmap_region(laby_texture, 72, 72, 12, 36, al_map_rgb(255,255,255), 0, 0, current_blue.x, current_blue.y+LABY_CASE_SIZE-2*LABY_WALL_SIZE, 1, 1, 0, 0);
            }
            if(((grid[y][x] == 6) && (y!=ROWS-1 && grid[y+1][x] == 5)) || ((grid[y][x] == 7) && (x != COLS-1 && grid[y][x+1] == 8)) || ((y!=ROWS-1 && grid[y+1][x] == 5) && (x != COLS-1 && grid[y][x+1] == 8))){ // A DROITE ET EN BAS
                al_draw_tinted_scaled_rotated_bitmap_region(laby_texture, 60, 72, 12, 36, al_map_rgb(255,255,255), 0, 0, current_blue.x+LABY_CASE_SIZE-LABY_WALL_SIZE, current_blue.y+LABY_CASE_SIZE-2*LABY_WALL_SIZE, 1, 1, 0, 0);
            }
        }
    }
